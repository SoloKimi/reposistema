...
$cmdR = "SELECT rr.jsonrespostas as jsonrespostas, c.titulo as colaborador, r.nomefantasia as nomefantasia
            FROM RelatorioRespostas rr
            LEFT JOIN Colaborador c ON rr.Colaborador_id = c.id
            LEFT JOIN Revenda r ON c.Revenda_id = r.id
            WHERE rr.id = '$idrr'";
$resultR = mysqli_query($link, $cmdR);
$dadosR = mysqli_fetch_array($resultR);

$jsonrespostas = $dadosR['jsonrespostas'];
...
<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="title" id="defaultModalLabel">Respostas de "<?= $dadosR['colaborador'] ?>" referente a revenda "<?= $dadosR['nomefantasia'] ?>" ao relatório "<?= $dados['titulo'] ?>"</h4>
        </div>
            ...
            <form class="forms-sample">
                <div class="form-group">
                        ...
                        <table class="table">
                            <thead>
                                <tr>
                                    ...
                                    <?php }else{ ?>
                                        <th style="width: 40%;text-align: center">Revenda referência</th>
                            ...
                            <tbody>
                                ...
                                ?>
                                        <tr>
                                            ...
                                            <?php
                                            $jsonBancoR = json_decode($jsonrespostas);
                    ...
                    <div class="add-items d-flex mb-0">
                        ...
                        <button type="button" class="btn btn-dark" onclick="exportarRelatorio('<?= $idrp; ?>', '<?= $idrr; ?>')">Exportar</button>
                    </div>
            </form>
        </div>
    </div>
